...
SUBTITLE_BG_HEIGHT = 160

def hex_to_rgb(h):
    h = h.lstrip('#')
    return tuple(int(h[i:i+2], 16) for i in (0, 2, 4))

def make_subtitle_bar(dur, text, text_color=C_WHITE, bg_alpha=0.85):
    """画面下部に帯付き字幕を作成"""
    # 半透明の黒帯
    bar = ColorClip(
        size=(WIDTH, SUBTITLE_BG_HEIGHT), 
        color=(20, 20, 20), 
        duration=dur
    ).with_opacity(bg_alpha).with_position((0, SUBTITLE_Y - 20))
    ...
    txt = TextClip(
        ...
        color=text_color, 
        font=FONT_BOLD
    ).with_position(('center', SUBTITLE_Y + 30)).with_duration(dur)
...
def make_scene(dur, bg, texts):
    ...
    # フェードを短く
    comp = comp.with_effects([CrossFadeIn(0.15), CrossFadeOut(0.15)])
    return comp

def make_mockup_scene(dur, bg, image_path, subtitle_text, img_scale=0.7, img_y_offset=-180):
    """モックアップ + 下部固定字幕"""
    bg_clip = ColorClip(size=(WIDTH, HEIGHT), color=hex_to_rgb(bg), duration=dur)
    clips = [bg_clip]
    ...
    if image_path.exists():
        ...
        max_h = int(HEIGHT * 0.55)  # モックアップは上半分に
...
def main():
    print(f"=== SendRight VSL v2（字幕改善版）===")
    ...
    scenes.append(make_mockup_scene(
        3.5, C_LIGHT, 
        MOCKUP_DIR / "pain-point.png",
        "こんな経験ありませんか？",
        img_scale=0.85, img_y_offset=-150
    ))
    ...
    scenes.append(make_mockup_scene(
        ...
        "貼り付け → 生成 → 送信",
        img_scale=0.9, img_y_offset=-120
    ))
    ...
    scenes.append(make_mockup_scene(
        ...
        "使った人から結果が出てる",
        img_scale=0.9, img_y_offset=-100
    ))
    ...
    scenes.append(make_mockup_scene(
        ...
        MOCKUP_DIR / "line-chat-success.png",
        "7日間無料で試せる",
        img_scale=0.55, img_y_offset=-200
    ))
    ...
    output_path = OUTPUT_DIR / f"vsl-v2-{FORMAT}.mp4"
    print("エンコード中...")
